...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        if mask is not None:
            ...
            #将mask为0的位置对其attention_scores相应位置使用-1e9来填充
...
def build_transformer(
    ...
    """
    src_vocab_size: int, 需要翻译的句子词表大小
    tgt_vocab_size: int, 对应翻译句子的词表大小
    src_seq_len: int, 对应翻译句子的长度
    tgt_seq_len: int, 对应目标句子的长度
    d_model: int=512,对应嵌入embed_dim大小
    N: int=6,对应encoder或者decoder中块重复多少
    h: int=8, 多头注意力中划分的head数量
    dropout: float=0.1,
    d_ff: int=2048：FFB中的神经元个数
    """
    ...
    for _ in range(N):
        ...
        encoder_block = EncoderBlock(
            features=d_model, self_attention_block=encoder_self_attention_block,
    ...
    for _ in range(N):
        ...
        decoder_block = DecoderBlock(
            features=d_model, self_attention_block=decoder_self_attention_block,
